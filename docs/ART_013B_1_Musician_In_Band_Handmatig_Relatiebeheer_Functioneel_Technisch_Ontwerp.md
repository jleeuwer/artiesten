...
## Fix-2: legacy schema compatibility

De productiepreflight liet zien dat de bestaande tabel de functionele foreign keys `mb_musician_key` en `mb_artist_key` bevat, maar nog geen zelfstandige relationsleutel. De eerdere ontwerpaanname dat `mb_musician_band_key` al bestond was onjuist.

De migratie voegt daarom zelf een `bigint GENERATED BY DEFAULT AS IDENTITY`-kolom `mb_musician_band_key` toe en borgt uniciteit met `uq_musician_in_band_relation_key`. Een eventueel reeds bestaande primary key wordt niet vervangen. Hierdoor blijft het bestaande schema behouden terwijl de API iedere relatie stabiel kan adresseren.
