  ...
  return (
    <div className="artist-list-identity-visual">
      <span className="artist-list-status-slot">
        {isDeceased ? (
          <span
            className="artist-deceased-indicator"
            title={`Artiest overleden${artist?.ar_artist_passing ? ` op ${fmtDate(artist.ar_artist_passing)}` : ""}`}
            aria-label="Artiest overleden"
            data-testid="artist-deceased-indicator"
          >
            <i className="bi bi-hourglass-bottom" aria-hidden="true"></i>
          </span>
        ) : null}
      </span>
      {showImage ? (
            ...
            {detailsArtist ? (
              <div className="small">
                ...
                <div className="mb-2 d-flex align-items-center gap-2">
                  ...
                  {Boolean(String(detailsArtist.ar_artist_passing || "").trim()) ? (
                    <span
                      className="artist-deceased-indicator artist-deceased-indicator-detail"
                      title={`Artiest overleden${detailsArtist.ar_artist_passing ? ` op ${fmtDate(detailsArtist.ar_artist_passing)}` : ""}`}
                      aria-label="Artiest overleden"
                      data-testid="artist-details-deceased-indicator"
                    >
                      <i className="bi bi-hourglass-bottom" aria-hidden="true"></i>
                      <span>Overleden</span>
                    </span>
