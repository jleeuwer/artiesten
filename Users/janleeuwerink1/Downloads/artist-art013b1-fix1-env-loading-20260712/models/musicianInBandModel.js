...
async function searchMusicians(q, limit=25) { return (await pool.query(`SELECT mu_musician_key AS musician_key, mu_musician_name AS musician_name, ar_artist_key AS artist_key FROM public.musician WHERE mu_musician_name ILIKE $1 ORDER BY mu_musician_name LIMIT $2`, [`%${String(q||'').trim()}%`, limit])).rows; }
...
module.exports={listForBand,listForPersonArtist,getById,searchMusicians,create,update,remove,findDuplicateOrOverlap};
